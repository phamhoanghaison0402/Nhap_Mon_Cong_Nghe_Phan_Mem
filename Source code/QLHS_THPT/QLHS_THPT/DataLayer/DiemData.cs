...
namespace QLHS_THPT.DataLayer
{
    public class DiemData
    {
        ...
        {
            SqlCommand cmd = new SqlCommand("SELECT * FROM BANGDIEM");
            m_DiemData.Load(cmd);
            return m_DiemData;
        }

        public DataTable LayDsDiem1()
        {
            SqlCommand cmd = new SqlCommand("SELECT DISTINCT BD.MaLoaiKT, TenLoaiKT FROM MONHOC MH,BANGDIEM BD, LOAIHINHKIEMTRA LD, LOP L, QUATRINHHOC QTH WHERE MH.MaMonHoc = BD.MaMonHoc AND BD.MaLoaiKT = LD.MaLoaiKT AND BD.MaQuaTrinhHoc = QTH.MaQuaTrinhHoc AND L.MaLop = QTH.MaLop");
        ...
        public DataTable LayDsDiem(String maHocSinh, String maMonHoc, String maHocKy, String maNamHoc, String maLop)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT *  FROM  QUATRINHHOC QTH, BANGDIEM D, HOCSINH S, MONHOC H, LOAIHINHKIEMTRA O, LOP L WHERE O.MaLoaiKT = D.MaLoaiKT AND QTH.MaQuaTrinhHoc = D.MaQuaTrinhHoc AND QTH.MaHocSinh = S.MaHocSinh AND D.MaMonHoc = H.MaMonHoc AND D.MaLoaiKT = O.MaLoaiKT AND  QTH.MaLop = L.MaLop AND  S.MaHocSinh = @maHocSinh AND   H.MaMonHoc = @maMonHoc AND QTH.MaHocKy = @maHocKy AND L.MaNamHoc = @maNamHoc AND QTH.MaLop = @maLop");
        ...
        public DataTable LayDsDiemCN(String maHocSinh, String maMonHoc, String maHocKy, String maNamHoc, String maLop, String maLoaiKT)
        {
            DataService m_DData = new DataService();

            SqlCommand cmd = new SqlCommand("SELECT *  FROM  QUATRINHHOC QTH, BANGDIEM D, HOCSINH S, MONHOC H, LOAIHINHKIEMTRA O, LOP L WHERE O.MaLoaiKT = D.MaLoaiKT AND QTH.MaQuaTrinhHoc = D.MaQuaTrinhHoc AND QTH.MaHocSinh = S.MaHocSinh AND D.MaMonHoc = H.MaMonHoc AND D.MaLoaiKT = O.MaLoaiKT AND  QTH.MaLop = L.MaLop AND  S.MaHocSinh = @maHocSinh AND   H.MaMonHoc = @maMonHoc AND QTH.MaHocKy = @maHocKy AND L.MaNamHoc = @maNamHoc AND QTH.MaLop = @maLop AND O.MaLoaiKT = @maLoaiKT");
            cmd.Parameters.Add("maHocSinh", SqlDbType.VarChar).Value = maHocSinh;
            cmd.Parameters.Add("maLop", SqlDbType.VarChar).Value = maLop;
            cmd.Parameters.Add("maMonHoc", SqlDbType.VarChar).Value = maMonHoc;
            cmd.Parameters.Add("maHocKy", SqlDbType.VarChar).Value = maHocKy;
            cmd.Parameters.Add("maNamHoc", SqlDbType.VarChar).Value = maNamHoc;
            cmd.Parameters.Add("maLoaiKT", SqlDbType.VarChar).Value = maLoaiKT;
        ...
        //Mới
        public void LuuDiem(String maQuaTrinhHoc, String maMonHoc, String maLoaiDiem, float diemSo)
        {
            DataService m_DData = new DataService();

            SqlCommand cmd = new SqlCommand("INSERT INTO BANGDIEM VALUES(@maQuaTrinhHoc, @maMonHoc, @maLoaiDiem, @diemSo)");
        ...
        public void XoaDiem(int stt)
        {
            ...
            SqlCommand cmd = new SqlCommand("DELETE FROM BAOCAOTONGKETMON WHERE MaBCTKMon = @stt");
            cmd.Parameters.Add("stt", SqlDbType.Int).Value = stt;
            
            m_DData.Load(cmd);
        }

        public void XoaBangDiem(int stt)
        {
            DataService m_DData = new DataService();

            SqlCommand cmd = new SqlCommand("DELETE FROM BANGDIEM WHERE STT = @stt");
            cmd.Parameters.Add("stt", SqlDbType.Int).Value = stt;
        ...
        public DataTable LayDsDiemHocSinh(String maHocSinh, String maMonHoc, String maHocKy, String maNamHoc, String maLop)
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT * " +
                                            "FROM DIEM D, LOAIHINHKIEMTRA L " +
                                            "WHERE D.MaLoaiKT = L.MaLoaiKT AND " +
        ...
        public DataTable LayDsDiemHocSinh(String maQuaTrinhHoc, String maMonHoc)
        {
            DataService m_DData = new DataService();

            SqlCommand cmd = new SqlCommand("SELECT * FROM QUATRINHHOC QTH, BANGDIEM D, LOAIHINHKIEMTRA L WHERE QTH.MaQuaTrinhHoc = D.MaQuaTrinhHoc AND D.MaLoaiKT = L.MaLoaiKT AND D.MaQuaTrinhHoc = @maQuaTrinhHoc AND D.MaMonHoc = @maMonHoc");
            cmd.Parameters.Add("maQuaTrinhHoc", SqlDbType.VarChar).Value = maQuaTrinhHoc;
            cmd.Parameters.Add("maMonHoc", SqlDbType.VarChar).Value = maMonHoc;
