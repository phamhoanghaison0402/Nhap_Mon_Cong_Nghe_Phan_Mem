...
namespace QLHS_THPT.Controller
{
    public class DiemCtrl
    {
        ...
        {
            m_DiemData.LuuDiem(maQuaTrinhHoc, maMonHoc, maLoaiDiem, diemSo);
        }
        #endregion
        ...
        public void HienThiDanhSachXemDiem(ListViewEx lVXemDiem, String maHocSinh, String maMonHoc, String maHocKy, String maNamHoc, String maLop)
        {
            ...
            foreach (DataRow row in m_DT.Rows)
            {
               ...
               // item.SubItems.Add(row["TenLoai"].ToString());
                //if (row["MaLoai"].ToString() == "LD001" || row["MaLoai"].ToString() == "LD002")
        ...
        public float DiemTrungBinhKiemTra(String maHocSinh, String maMonHoc, String maHocKy, String maNamHoc, String maLop)
        {
            ...
            foreach (DataRow row in m_DT.Rows)
            {
                if (row["MaLoaiKT"].ToString() != "LD0004")
                {
        ...
        //Mới
        public float DiemTrungBinhMon(String maQuaTrinhHoc, String maMonHoc)
        {
            DataTable m_DT = m_DiemData.LayDsDiemHocSinh(maQuaTrinhHoc, maMonHoc);
        ...
        public float DiemTrungBinhChungCacMonHocKy(String maQuaTrinhHoc)
        {
          ...
          //  DataTable m_DT = m_MonHocData.LayDsMonHoc(maNamHoc, maLop);
            DataTable m_DT = m_ChuongTrinhHocData.LayDsChuongTrinhHoc();
            
            foreach (DataRow row in m_DT.Rows)
            {
                diemTBTungMon   =   DiemTrungBinhMon(maQuaTrinhHoc, row["MaMonHoc"].ToString());
